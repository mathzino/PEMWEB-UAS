...
if (isset($_GET["id_transaction"])) {
  ...
  $getNotifyDetailQuery = "SELECT transaction.id_transaction, transaction.status,  toko.name as store_name, transaction_detail.qt,product.name as product_name,product.price,toko.image_profile, code.code,code.claim,product.image FROM transaction  INNER join toko on transaction.toko_id=toko.toko_id INNER join transaction_detail on transaction_detail.id_transaction=transaction.id_transaction inner join product on transaction_detail.id_product=product.product_id inner join code ON code.id_transaction = transaction.id_transaction where transaction.id_transaction =  '$id_transaction' ";
  $getCardInformation = mysqli_query(connection(), $getNotifyDetailQuery);
  $transactionInformationArr = $getCardInformation->fetch_assoc();
  $transactionInformation = $transactionInformationArr;
  $total = 0;
          ...
          <div style="margin-top: 0.5rem">
            <div style="display: flex; justify-content: space-between">
              <div style="width: 6rem; height: 1.75rem; background-color: #dbeafe; border-radius: 1.125rem">
                <a href="/seller"
                  style="background: #3b82f6; padding: 16px 6px; text-align: center; border-radius: 24px;text-decoration:none">
                  <p
        ...
        <div style="width: 100%; height: 100vh; background-color: #ffffff; border-radius: 3rem">
          <div
            ...
            <div style="display: flex; gap: 8px">
              <a href="notify.php"><svg width="20" height="18" viewBox="0 0 20 18" fill="none"
                  xmlns="http://www.w3.org/2000/svg">
                  <path d="M6 1L1 6L6 11" stroke="black" stroke-width="2" stroke-linecap="round"
                    stroke-linejoin="round" />
                  <path d="M1 6H9C14.523 6 19 10.477 19 16V17" stroke="black" stroke-width="2" stroke-linecap="round"
                    stroke-linejoin="round" />
                </svg>
              </a>
              <p
          ...
          <div style="padding: 24px 36px">
            <div
              style="padding: 16px; background: #ffffff; box-shadow: 0px 0px 16px rgba(0, 0, 0, 0.1); border-radius: 8px">
              <div style="display: flex; gap: 20px">
                <div
                  style="width: 88px; height: 88px; border-radius: 100%; background-color: #cbcbcb; position: relative; overflow: hidden">
                  <img style="object-fit: cover; width: 100%; height: 100%"
                    src="../seller/assets/toko/<?= $transactionInformation['image_profile']?>"
                    alt="toko" />
                </div>
                <div style="display: flex; flex-direction: column; justify-content: center">
                  <div style="display: flex; gap: 4px; align-items: center">
                    ...
                    <div
                      ...
                      <?php echo $transactionInformation['store_name'] ?></div>
                  </div>
                  <div
                    style="font-family: 'Inter'; font-style: normal; font-weight: 700; font-size: 16px; line-height: 19px; color: #3b82f6; margin-top: 12px; margin-bottom: 7px">
              ...
              <div style="
                    ...
                    padding: 8px 0;
                  ">
                <?php if($transactionInformation['status'] == 0):?>
                Menunggu Konfirmasi
                <?php elseif($transactionInformation['status'] == 1):?>
                Pesanan telah disetujui, silahkan ditunggu..
                <?php elseif($transactionInformation['status'] == 2):?>
                  <form action="ratingPage.php" method="GET">
                  <input type="hidden" name="id_transaction" value="<?php echo $transactionInformation['id_transaction']?>">
                    <button type="submit" style="border:none;background-color:transparent;color:white;cursor:pointer;">Beri Rating</button>
                  </form>
                <?php elseif($transactionInformation['status'] == 3):?>
                Ditolak
                <?php endif;?>
              </div>
              ...
              <div
                style="font-family: 'Inter'; font-style: normal; font-weight: 400; font-size: 16px; line-height: 19px; color: #3b82f6; text-align: center; margin-top: 20px">
                Kode konfirmasi</div>

              <div
                style="font-family: 'Inter'; font-style: normal; font-weight: 700; font-size: 40px; line-height: 48px; color: #3b82f6; text-align: center; margin-top: 4px">
                <?php echo $transactionInformation['code'] ?></div>

              <div
                style="font-family: 'Inter'; font-style: normal; font-weight: 400; font-size: 16px; line-height: 19px; color: #3b82f6; margin-top: 24px">
                Detail belanjaan :</div>


                <?php
                foreach($getCardInformation as $cardInformation){
                    ?>
                    
                    <div style="margin-top: 16px"> 
                <div
                  style="background: #ffffff; box-shadow: 0px 0px 16px rgba(0, 0, 0, 0.1); border-radius: 8px; display: flex; gap: 20px; overflow: hidden">
                  <div style="width: 114px; height: 98px; background-color: #bfbfbf; position: relative">
                    <img
                      src=  '<?php echo "../assets/produk/" . $cardInformation['image']?>'
                  ...
                  <div style="display: flex; flex-direction: column; justify-content: center">
                    <div style="display: flex; gap: 4px; align-items: center">
                      ...
                      <div
                        style="font-family: 'Inter'; font-style: italic; font-weight: 400; font-size: 8px; line-height: 10px; color: #3b82f6">
                        <?php echo $cardInformation['store_name']?></div>
                    </div>
                    <div
                      style="font-family: 'Inter'; font-style: normal; font-weight: 400; font-size: 10px; line-height: 12px; color: #3b82f6; margin-top: 12px; margin-bottom: 7px">
                      <?php echo $cardInformation['product_name']?></div>
                    <div
                      style="font-family: 'Inter'; font-style: normal; font-weight: 700; font-size: 12px; line-height: 15px; color: #3b82f6">
                      Rp <?php echo $cardInformation['price']*$cardInformation['qt']?></div>
                  </div>
                </div>
              </div>
            </div>   
